...
#pragma region 到字节集
std::vector<unsigned char> 到字节集(const std::string& 欲转换为字节集的数据);
std::vector<unsigned char> 到字节集(const std::wstring& 欲转换为字节集的数据);
std::vector<unsigned char> 到字节集(unsigned char 欲转换为字节集的数据);
std::vector<unsigned char> 到字节集(short 欲转换为字节集的数据);
std::vector<unsigned char> 到字节集(long long 欲转换为字节集的数据);
std::vector<unsigned char> 到字节集(float 欲转换为字节集的数据);
std::vector<unsigned char> 到字节集(double 欲转换为字节集的数据);
std::vector<unsigned char> 到字节集(std::vector<unsigned char> 欲转换为字节集的数据);
std::vector<unsigned char> 到字节集(std::vector<int> 欲转换为字节集的数据);
std::vector<unsigned char> 到字节集(std::vector<short> 欲转换为字节集的数据);
std::vector<unsigned char> 到字节集(std::vector<long long> 欲转换为字节集的数据);
std::vector<unsigned char> 到字节集(std::vector<double> 欲转换为字节集的数据);
std::vector<unsigned char> 到字节集(std::vector<float> 欲转换为字节集的数据);
std::vector<unsigned char> 到字节集(char* 欲转换为字节集的数据);
std::vector<unsigned char> 到字节集(const char* 欲转换为字节集的数据);
std::vector<unsigned char> 到字节集(const wchar_t* 欲转换为字节集的数据);
std::vector<unsigned char> 到字节集(wchar_t* 欲转换为字节集的数据);

/*****************万物皆可机器码 *************/
template<typename Type>
std::vector<unsigned char> 到字节集(const std::vector<Type>& 欲转换为字节集的数据) {
	if (欲转换为字节集的数据.empty()) {
		return {};
	}
	return	std::vector<unsigned char>((unsigned char*)欲转换为字节集的数据.data(), ((unsigned char*)欲转换为字节集的数据.data() + 欲转换为字节集的数据.size() * sizeof(Type)));
};
template<typename Type>
std::vector<unsigned char> 到字节集(const Type& 欲转换为字节集的数据) {//大型结构体防止拷贝，保证数据结构的执行效率
	return  std::vector<unsigned char>((unsigned char*)&欲转换为字节集的数据, ((unsigned char*)&欲转换为字节集的数据 + sizeof(Type)));
};
/*****************万物皆可机器码 *************/
#pragma endregion
/*C中作用很小的几个宏*/
...
size_t 取字节集长度(const std::vector<unsigned char>& 字节集数据);
...
*/
std::vector<unsigned char> 取字节集左边(const std::vector<unsigned char>& 欲取其部分的字节集, size_t 欲取出字节的数目);
/* 调用格式： 〈字节集〉 取字节集右边 （字节集 欲取其部分的字节集，整数型 欲取出字节的数目） - 系统核心支持库->字节集操作
   ...
   操作系统需求： Windows、Linux*/
std::vector<unsigned char> 取字节集右边(const std::vector<unsigned char>& 欲取其部分的字节集, size_t 欲取出字节的数目);
/*    调用格式： 〈字节集〉 取字节集中间 （字节集 欲取其部分的字节集，整数型 起始取出位置，整数型 欲取出字节的数目） - 系统核心支持库->字节集操作
   ...
   操作系统需求： Windows、Linux
*/
std::vector<unsigned char> 取字节集中间(const std::vector<unsigned char>& 欲取其部分的字节集, size_t 起始取出位置, size_t 欲取出字节的数目);
/*  调用格式： 〈字节集〉 字节集替换 （字节集 欲替换其部分的字节集，整数型 起始替换位置，整数型 替换长度，［字节集 用作替换的字节集］） - 系统核心支持库->字节集操作
   ...
   操作系统需求： Windows、Linux*/
std::vector<unsigned char> 字节集替换(const std::vector<unsigned char>& 欲替换其部分的字节集, size_t 起始替换位置, size_t 替换长度, const std::vector<unsigned char>& 用作替换的字节集);
/*
...
*/
std::vector<unsigned char> 子字节集替换(const std::vector<unsigned char>& 欲被替换的字节集, const std::vector<unsigned char>& 欲被替换的子字节集, const std::vector<unsigned char>& 用作替换的子字节集 = std::vector<unsigned char>(), size_t 进行替换的起始位置 = 1);
/*
   调用格式： 〈字节集〉 子字节集替换 （字节集 欲被替换的字节集，字节集 欲被替换的子字节集，［字节集 用作替换的子字节集］，［整数型 进行替换的起始位置］，［整数型 替换进行的次数］） - 系统核心支持库->字节集操作
   英文名称：RpSubBin
   返回一个字节集，该字节集中指定的子字节集已被替换成另一子字节集，并且替换发生的次数也是被指定的。本命令为初级命令。
   ...
   参数<5>的名称为“替换进行的次数”，类型为“整数型（intptr_t）”，可以被省略。对子字节集进行替换的次数。如果省略，默认进行所有可能的替换。
*/
std::vector<unsigned char> 子字节集替换(std::vector<unsigned char> 欲被替换的字节集, std::vector<unsigned char> 欲被替换的子字节集, std::vector<unsigned char> 用作替换的子字节集, size_t 进行替换的起始位置, size_t 替换进行的次数);
...
std::vector<unsigned char> 指针到字节集(void* 内存数据指针, size_t 内存数据长度);


int 取字节集内整数(const std::vector<unsigned char>& 待处理的字节集, size_t 欲获取整数所处偏移, bool 是否反转字节序 = false);
void 置字节集内整数(std::vector<unsigned char> 待处理的字节集, size_t 欲设置整数所处偏移, int 欲设置的整数值, bool 是否反转字节序);
std::vector<unsigned char>  取空白字节集(size_t  零字节数目);
...
/*    调用格式： 〈整数型〉 寻找字节集 （字节集 被搜寻的字节集，字节集 欲寻找的字节集，［整数型 起始搜寻位置］） - 系统核心支持库->字节集操作
   ...
   操作系统需求： Windows、Linux
*/
size_t 寻找字节集(const std::vector<unsigned char>& 被搜寻的字节集, const std::vector<unsigned char>& 欲寻找的字节集, size_t 起始搜寻位置 = 1);
...
/*    调用格式： 〈整数型〉 寻找字节集 （字节集 被搜寻的字节集，字节集 欲寻找的字节集，［整数型 起始搜寻位置］） - 系统核心支持库->字节集操作
   ...
   操作系统需求： Windows、Linux
*/
intptr_t 寻找字节集下标(const std::vector<unsigned char>& 被搜寻的字节集, const std::vector<unsigned char>& 欲寻找的字节集, std::optional<size_t> 起始搜寻位置 = std::nullopt);
